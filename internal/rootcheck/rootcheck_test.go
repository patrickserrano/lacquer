...
func TestVerifyRefusesUntrackedContent(t *testing.T) {
	root := pinnedClone(t, "3.0.0")
	write(t, filepath.Join(root, "extra-template.md"), "unreviewed")
	if err := Inspect(root, false).Verify(); err == nil {
		t.Fatal("untracked content accepted as a release")
	}
}

func TestVerifyAcceptsMatchingTagAmongMultipleTags(t *testing.T) {
	root := pinnedClone(t, "3.0.0")
	run(t, root, "git", "tag", "-a", "unrelated", "-m", "another tag")
	if err := Inspect(root, false).Verify(); err != nil {
		t.Fatal(err)
	}
}

func TestVerifyRefusesWhenGitUnavailable(t *testing.T) {
	root := pinnedClone(t, "3.0.0")
	t.Setenv("PATH", t.TempDir())
	if err := Inspect(root, false).Verify(); err == nil {
		t.Fatal("missing git accepted as verified")
	}
}

func TestVerifyRefusesWhenStatusFails(t *testing.T) {
	root := pinnedClone(t, "3.0.0")
	...
	dir := t.TempDir()
	script := "#!/bin/sh\nif [ \"$3\" = status ]; then exit 1; fi\nexec '" + realGit + "' \"$@\"\n"
	if err := os.WriteFile(filepath.Join(dir, "git"), []byte(script), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", dir)
	if err := Inspect(root, false).Verify(); err == nil {
		t.Fatal("failed status accepted as clean")
	}
}

func TestVerifyRefusesContentNestedInPinnedRepository(t *testing.T) {
	root := pinnedClone(t, "3.0.0")
	nested := filepath.Join(root, "nested")
	write(t, filepath.Join(nested, "VERSION"), "3.0.0\n")
	if err := Inspect(nested, false).Verify(); err == nil {
		t.Fatal("nested copy borrowed parent pin")
	}
}
