...
// preCommitHook is the fingerprinted head of the hook `pre-commit install`
// writes, as found in rail's .git/hooks/pre-commit.old.
const preCommitHook = "#!/usr/bin/env bash\n# File generated by pre-commit: https://pre-commit.com\nexec pre-commit hook-impl\n"

// lefthookBridge is the shape of the `ios-pre-commit` command the web and
// supabase fragments ship (#317): lefthook calling pre-commit.
const lefthookBridge = "pre-commit:\n  commands:\n    ios-pre-commit:\n      run: |\n        [ -f .pre-commit-config.yaml ] || exit 0\n        pre-commit run --hook-stage pre-commit --files {staged_files}\n"

// The measured case: rail and momfriend. Their .git/hooks were installed by
// pre-commit before the bridge existed, and the lefthook.yml lacquer later
// composed CALLS pre-commit. lefthook is then always the right owner, and the
// report has to say what to run -- an operator reading the generic "whichever
// manager guards code that exists" line had to ask.
func TestAPreCommitHookUnderABridgingLefthookGetsTheInstallRemedy(t *testing.T) {
	dir := gitRepo(t)
	write(t, dir, ".pre-commit-config.yaml", "repos: []\n")
	write(t, dir, "lefthook.yml", lefthookBridge)
	write(t, dir, ".git/hooks/pre-commit", preCommitHook)

	fs := Check(dir)
	if len(fs) != 1 {
		t.Fatalf("expected exactly the lefthook finding, got %+v", fs)
	}
	f := fs[0]
	if f.Manager != "lefthook" || f.Rival != "pre-commit" {
		t.Fatalf("wrong finding: %+v", f)
	}
	if !f.BridgesRival {
		t.Fatalf("a lefthook.yml that calls `pre-commit run` was not detected as bridging the installed rival: %+v", f)
	}
	if f.Bridged {
		t.Fatalf("pre-commit's config does not call lefthook, so lefthook's rules do NOT run: %+v", f)
	}

	out := Format(fs)
	for _, want := range []string{
		"lefthook install",                // the concrete remedy
		"repository root",                 // where to run it
		"CALLS pre-commit",                // why it is safe
		"both rule sets",                  // what the reader keeps
		"do NOT run `pre-commit install`", // how to undo it by accident
	} {
		if !strings.Contains(out, want) {
			t.Errorf("remedy is missing %q:\n%s", want, out)
		}
	}
	// The generic lines are what this replaces: they left the reader to work out
	// the owner, and "do not reinstall lefthook" is the opposite of the fix here.
	for _, gone := range []string{"guards code that actually EXISTS", "DO NOT reflexively reinstall"} {
		if strings.Contains(out, gone) {
			t.Errorf("report still carries the generic collision line %q:\n%s", gone, out)
		}
	}
}

// Detected, not assumed. A lefthook.yml that does not call pre-commit -- older
// lacquer content, or a hand-written one -- must keep the generic guidance:
// telling that repo to run `lefthook install` would silently drop its iOS gates.
func TestAPreCommitHookUnderANonBridgingLefthookKeepsTheGenericGuidance(t *testing.T) {
	for name, lefthook := range map[string]string{
		"no bridge": "pre-commit:\n  commands:\n    biome:\n      run: biome ci\n",
		// Prose about the tool is not an invocation of it (#378's lesson): a
		// comment naming `pre-commit run` bridges nothing.
		"only a comment": "pre-commit:\n  commands:\n    # we do not call pre-commit run from here\n    biome:\n      run: biome ci\n",
	} {
		t.Run(name, func(t *testing.T) {
			dir := gitRepo(t)
			write(t, dir, ".pre-commit-config.yaml", "repos: []\n")
			write(t, dir, "lefthook.yml", lefthook)
			write(t, dir, ".git/hooks/pre-commit", preCommitHook)

			fs := Check(dir)
			if len(fs) != 1 || fs[0].Manager != "lefthook" || fs[0].Rival != "pre-commit" {
				t.Fatalf("expected the lefthook collision finding, got %+v", fs)
			}
			if fs[0].BridgesRival {
				t.Fatalf("a lefthook.yml that never calls pre-commit was read as bridging it: %+v", fs[0])
			}
			out := Format(fs)
			if !strings.Contains(out, "guards code that actually EXISTS") {
				t.Errorf("the unbridged collision lost its generic guidance:\n%s", out)
			}
			if strings.Contains(out, "lefthook install") {
				t.Errorf("told a repo whose lefthook does not call pre-commit to run `lefthook install`, "+
					"which would drop its iOS gates:\n%s", out)
			}
		})
	}
}

// The remedy is only for the lacquer's OWN other manager. A hand-written hook
// under a bridging lefthook.yml is unknown -- it may be the project's own
// enforcement -- so `lefthook install` over it is not something to recommend.
func TestAHandWrittenHookUnderABridgingLefthookIsNotGivenTheRemedy(t *testing.T) {
	dir := gitRepo(t)
	write(t, dir, ".pre-commit-config.yaml", "repos: []\n")
	write(t, dir, "lefthook.yml", lefthookBridge)
	write(t, dir, ".git/hooks/pre-commit", "#!/bin/sh\n# our own pre-commit hook\n./scripts/check.sh\n")

	for _, f := range Check(dir) {
		if f.BridgesRival {
			t.Errorf("%s was read as bridging a rival that is not installed: %+v", f.Manager, f)
		}
	}
	out := Format(Check(dir))
	if strings.Contains(out, "lefthook install") {
		t.Errorf("recommended `lefthook install` over a hook the lacquer did not write:\n%s", out)
	}
	if !strings.Contains(out, "may be deliberate") {
