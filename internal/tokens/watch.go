    ...
    steps:
      ...
      - name: Create an UNPAIRED watch simulator
        ...
        run: |
          ...
          # This workspace's strays, then the pipeline itself: the step shell's other
          # children are exactly xcodebuild, tee and xcbeautify, plus this watchdog.
          # Killing xcodebuild alone is not enough; a helper that inherited its output
          # pipe keeps tee waiting, and the step hangs until its timeout.
          ...
          # sim_device_crash_reports <device-id> <since-file>: every crash report, one
          # path per line, written after <since-file>'s mtime by an app INSTALLED ON
          # <device-id>. The host's DiagnosticReports is shared by every repository on
          # the Mac, so both filters matter: the device id appears in a report's procPath
          # (/Devices/<id>/data/Containers/Bundle/Application/...) only for an app on
          # that very device, which also excludes this same app's reports from another
          # run; the time window excludes this device's reports from before it. procPath
          # is read as JSON because ReportCrash escapes its slashes ("\/Users\/...").
          sim_device_crash_reports() {
            local dev=$1 since=$2 f path
            ...
            while IFS= read -r f; do
              ...
              case "$path" in
                */Devices/"$dev"/data/Containers/Bundle/Application/*) echo "$f" ;;
              esac
            done < <(find "$SIM_TEST_CRASH_DIR" -maxdepth 1 -name '*.ips' -newer "$since" 2>/dev/null)
          }

          # sim_test_host_crash <since-file>: one crash report of the app under test on
          # THIS run's device during this attempt, or nothing.
          #
          ...
          # an app bug.
          sim_test_host_crash() {
            sim_device_crash_reports "$SIM_TEST_DEVICE_ID" "$1" | head -n 1
          }
