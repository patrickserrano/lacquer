...
func usage(w io.Writer) {
	...
	fmt.Fprintln(w, "                               It also lists App Store versions rejected 3h, a valid build attached to no")
	fmt.Fprintln(w, "                               version 3h, and a version waiting for review 24h, from asc-snapshot.json")
	fmt.Fprintln(w, "                               beside the inbox file, which fleet-ops asc-status writes (lacquer holds no")
	fmt.Fprintln(w, "                               App Store Connect credentials and never calls it). A missing, unreadable or")
	fmt.Fprintln(w, "                               90-minute-old snapshot says \"couldn't check\", never nothing stuck; see")
	fmt.Fprintln(w, "                               docs/asc-snapshot.md.")
