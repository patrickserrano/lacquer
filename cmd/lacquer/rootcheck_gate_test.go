...
func pinnedRoot(t *testing.T, version string) string {
	...
	gittest.Init(t, origin, "-q", "--initial-branch=main")
...
func TestStatusSucceedsAgainstAPinnedRootAndStampsTheRootPath(t *testing.T) {
	...
	if !strings.Contains(errb.String(), root) {
		t.Errorf("stderr does not stamp the resolved root PATH — the one variable that actually changes "+
	...
	if strings.Contains(errb.String(), " HEAD ") || strings.Contains(errb.String(), "@ HEAD") {
		t.Errorf("stderr still renders the detached ref as the bare literal HEAD instead of resolving "+
			"it to the pinned tag:\n%s", errb.String())
	}
	if !strings.Contains(errb.String(), "v9.9.9") {
		t.Errorf("stderr does not name the resolved tag v9.9.9:\n%s", errb.String())
	}
}
...
func TestEveryLacquerRootCommandRefusesAnUnverifiedRoot(t *testing.T) {
	...
	for _, command := range []string{
		"init", "onboard", "adopt", "sync", "doctor", "fix",
		"plugins", "audit", "status", "version", "fleet", "console",
	} {
		args := []string{command}
...
func TestStatusDistinguishesStampOnlyDrift(t *testing.T) {
	root := realLacquer(t)
	project := fixtureProject(t, root)
	chdir(t, project)
	env := envMap(map[string]string{"LACQUER_ROOT": root})
	var out, errb bytes.Buffer
	...
	body = bytes.ReplaceAll(body, []byte("start v"+strings.TrimSpace(string(version))), []byte("start v0.0.1"))
	...
	if !strings.Contains(out.String(), "stamp-behind") || !strings.Contains(out.String(), "Only version stamps are behind; managed content matches") {
		t.Fatalf("stamp-only drift not explained:\n%s", out.String())
	}
	body = bytes.ReplaceAll(body, []byte("start v0.0.1 -->"), []byte("start v0.0.1 -->\nlocal edit"))
	if err := os.WriteFile(path, body, 0o644); err != nil {
		t.Fatal(err)
	}
	out.Reset()
	...
	if strings.Contains(out.String(), "Only version stamps are behind") {
		t.Fatalf("content edit misreported as stamp-only:\n%s", out.String())
	}
}
