...
If this repo also contains an iOS component, the iOS profile syncs a `.pre-commit-config.yaml` and this profile syncs a `lefthook.yml` — both write `.git/hooks`, and whichever `install`s last silently wins. Don't install both. lefthook owns `.git/hooks`: the `lefthook.yml` this profile syncs carries an `ios-pre-commit` command that runs `pre-commit run --hook-stage pre-commit`, so one installed hook runs the Supabase and iOS checks together, and it no-ops in a repo with no `.pre-commit-config.yaml`.
:::
